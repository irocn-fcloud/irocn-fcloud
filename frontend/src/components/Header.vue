...
export default {
  ...
  computed: {
    ...
    showPlayButton (){
      let num = this.selected.length === 0 ? -1 : this.selected[0]
      return this.isFiles && this.selectedCount === 1 && (num !== -1 &&(
        this.req.items[num].type === "image"
          ? true
          : (this.req.items[num].type === "video"
            ? true
            : false)))
    }
  },
